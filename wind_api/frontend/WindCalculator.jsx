...
/* ── API wiring ── */
const WIND_API_BASE = (typeof window !== "undefined" && window.__WIND_API_BASE__) || "/api/v1";

async function apiPost(path, body) {
  const resp = await fetch(WIND_API_BASE + path, {
    method: "POST",
  ...
  if (!resp.ok) {
    ...
    throw new Error((err.detail && (typeof err.detail === "string" ? err.detail : JSON.stringify(err.detail))) || `API error ${resp.status}`);
  }
  return resp.json();
}

function buildTopography(kztInputs) {
  // Map frontend topo_type ("2d_escarp") to API enum ("2d_escarpment")
  const topoType = kztInputs.topo_type === "2d_escarp" ? "2d_escarpment" : kztInputs.topo_type;
  return {
    topo_type: topoType,
    hill_height_ft:      kztInputs.H_ft,
    half_hill_length_ft: kztInputs.Lh_ft,
    dist_from_crest_ft:  kztInputs.x_ft,
    is_upwind:           kztInputs.upwind,
...
async function apiQz(P) {
  ...
  const heights = [...new Set([...defZ(g.h_ft), ...(g.extraHeights || []).filter(v => v > 0)])].sort((a, b) => a - b);

  const resp = await apiPost("/calculate/wind/qz", {
    project: {
      code_version: p.code_version, risk_category: p.risk_category,
      V_mph: p.V_mph, exposure: p.exposure,
      enclosure: p.enclosure || "enclosed", importance_factor: p.importance_factor || 1,
    },
    geometry: {
      L_ft: g.L_ft, B_ft: g.B_ft, h_ft: g.h_ft,
      roof_type: g.roof_type, roof_angle_deg: g.roof_angle_deg,
      parapet_height_ft: g.parapet_height_ft || 0,
    },
    topography: buildTopography(kztInputs),
    kd,
    z_heights_ft: heights,
  });

  const lastP = resp.pressures[resp.pressures.length - 1];
  return {
    code_version: resp.code_version, V_mph: resp.V_mph, exposure: resp.exposure,
    pressures: resp.pressures.map(r => ({
      z_ft: r.z_ft, kz: r.kz, kzt: r.kzt, qz_psf: r.qz_psf,
      alpha: r.alpha, zg_ft: r.zg_ft, ke: r.ke, kd: r.kd,
    })),
    kztH: lastP.kzt,
  };
}
...
async function apiDir(P) {
  ...
  // Compute G for display (always in JS; also passed as override for flexible/rigid_calc modes)
  ...
  const resp = await apiPost("/calculate/wind/mwfrs/directional", {
    project: {
      code_version: p.code_version, risk_category: p.risk_category,
      V_mph: p.V_mph, exposure: p.exposure,
      enclosure: p.enclosure || "enclosed", importance_factor: p.importance_factor || 1,
    ...
    topography: buildTopography(kztInputs),
    kd,
    gust_factor_G: gustInputs.mode === "rigid_fixed" ? null : Math.max(gRes.G, 0.85),
    z_heights_ft: heights,
  });

  const G    = resp.G;
  const qh   = resp.qh_psf;
  const gcpi = resp.gcpi;
  const cLW_n = resp.cp_leeward_wall_normal;
  const cLW_p = resp.cp_leeward_wall_parallel;
  ...
  const pLW_n = qh * G * cLW_n;
  const pLW_p = qh * G * cLW_p;
  const pSW   = qh * G * -0.7;
  const pWW   = qh * G * 0.8;
  const pR_n  = qh * G * roofNormal[0].cp;

  const lwPrs = {
    normal:   { pN: r2(pLW_n - qh*gcpi), pP: r2(pLW_n + qh*gcpi) },
    parallel: { pN: r2(pLW_p - qh*gcpi), pP: r2(pLW_p + qh*gcpi) },
  };

  // Map API profile; combined_normal/parallel already have GCpi cancelled (§27 combined WW+LW)
  const profile = resp.windward_wall_profile.map(r => ({
    z_ft: r.z_ft, kz: r.kz, kzt: kztH,
    pN: r.p_with_neg_gcpi, pP: r.p_with_pos_gcpi,
    combN_normal: r.combined_normal, combP_normal: r.combined_normal,
    combN_parallel: r.combined_parallel, combP_parallel: r.combined_parallel,
  }));
  ...
  return {
    ...
    cWW: 0.8, cSW: -0.7,
    cLW_n, ratioLW_n: r4(bl), ratioRoof_n: r4(hb), roofNormal,
    lwP_n: r2(pLW_n - qh*gcpi), lwN_n: r2(pLW_n + qh*gcpi),
    cLW_p, ratioLW_p: r4(lb), ratioRoof_p: r4(hl), roofParallel,
    lwP_p: r2(pLW_p - qh*gcpi), lwN_p: r2(pLW_p + qh*gcpi),
    swP: r2(pSW - qh*gcpi), swN: r2(pSW + qh*gcpi),
    profile,
    parWW: resp.parapet_windward_psf, parLW: resp.parapet_leeward_psf,
    oh: resp.overhang_soffit_psf,
...
async function apiLR(P) {
  ...
  const resp = await apiPost("/calculate/wind/mwfrs/lowrise", {
    project: {
      code_version: p.code_version, risk_category: p.risk_category,
      V_mph: p.V_mph, exposure: p.exposure,
      enclosure: p.enclosure || "enclosed", importance_factor: p.importance_factor || 1,
  ...
  if (!resp.is_applicable) {
    return { ok: false, reason: resp.inapplicable_reason, qh: 0, gcpi: resp.gcpi, ez: 0, cA: [], cB: [], pww: 0, plw: 0, sd: null };
  }

  const mapCase = (arr) => arr.map(r => ({
    zone: r.zone, gcpf: r.gcpf, pN: r.p_with_neg_gcpi, pP: r.p_with_pos_gcpi,
  }));

  let sd = null;
  if (resp.simple_diaphragm) {
    const s = resp.simple_diaphragm;
    sd = {
      a: s.a_ft, endZone2a: s.end_zone_2a_ft,
      transverse:  { intWall: s.int_wall_transverse, endWall: s.end_wall_transverse, intRoof: s.int_roof_transverse, endRoof: s.end_roof_transverse },
      longitudinal: { intWall: s.int_wall_longitudinal, endWall: s.end_wall_longitudinal },
    };
  }

  return {
    ok: true, reason: "",
    qh: resp.qh_psf, gcpi: resp.gcpi, ez: resp.end_zone_width_ft,
    cA: mapCase(resp.case_a), cB: mapCase(resp.case_b),
    pww: resp.parapet_windward_psf, plw: resp.parapet_leeward_psf,
    sd,
  };
}
async function apiCC(P) {
  ...
  const allZones = ["1", "1p", "2", "3", "oh1", "oh2", "oh3", "4", "5"];
  const allAreas = [10, 100, 200, 500, 1000];

  const resp = await apiPost("/calculate/wind/cc", {
    project: {
      code_version: p.code_version, risk_category: p.risk_category,
      V_mph: p.V_mph, exposure: p.exposure,
      enclosure: p.enclosure || "enclosed", importance_factor: p.importance_factor || 1,
    ...
    kd,
    zones: allZones,
    eff_wind_areas_sf: allAreas,
  });

  const qh   = resp.qh_psf;
  const gcpi = resp.gcpi;
  const a    = resp.zone_dimension_a_ft;
  const proc = g.h_ft <= 60 ? "h\u226460'" : "h>60'";

  const prs = resp.pressures.map(r => ({
    zone: r.zone,
    area: r.eff_wind_area_sf,
    gn: r.gcp_negative,
    gp: r.gcp_positive,
    isOverhang: r.zone.startsWith("oh"),
    pnN: r.p_neg_with_pos_gcpi,  // max suction = GCp_neg − GCpi
    ppP: r.p_pos_with_neg_gcpi,  // max positive = GCp_pos + GCpi
  }));

  const parAreas = [10, 20, 50, 100, 200, 500];
  let parPrs;
  if (resp.parapet_pressures && resp.parapet_pressures.length > 0) {
    parPrs = resp.parapet_pressures.map(r => ({
      area: r.eff_wind_area_sf,
      caseA:    r.p_case_a_psf,
      caseBint: r.p_case_b_int_psf,
      caseBcor: r.p_case_b_cor_psf,
    }));
  } else {
    // Fallback: compute parapet pressures client-side when parapet_height_ft = 0
    const PAR_GCpn = {
      caseA:    [[10, 3.1948], [500, 2.0262]],
      caseBint: [[10, -1.8876], [500, -1.3483]],
      caseBcor: [[10, -2.1573], [500, -1.3483]],
    };
    parPrs = parAreas.map(ar => ({
      area: ar,
      caseA:    r2(qh * interpGCp(ar, PAR_GCpn.caseA)),
      caseBint: r2(qh * interpGCp(ar, PAR_GCpn.caseBint)),
      caseBcor: r2(qh * interpGCp(ar, PAR_GCpn.caseBcor)),
    }));
  }

  return { qh: r2(qh), gcpi, a, prs, proc, theta: g.roof_angle_deg, roof: g.roof_type, parPrs, parAreas };
}
